...
def get_ini_filenames(additional_paths=None, use_importer=True, local=True):
    ...
    local_feedin_germany = os.path.join(os.path.expanduser("~"), '.feedin_germany')
    if os.path.isdir(local_feedin_germany) and local is True:
        paths.append(local_feedin_germany)
...
def add_config_files(files=None, paths=None):
    ...
    FILES.extend(files)
    cfg.read(files)
    set_feedin_germany_paths(paths)
...
def set_feedin_germany_paths(paths=None):
    # initialise de21 configuration
    logging.info('Loading feedin_germany configuration....')
    ...
    if datapath is None:
        datapath = os.path.join(os.path.expanduser("~"), 'feedin_germany') 
